...
def sample_opti_sequenz(iter=None, maxIteration = 501):
    ...
    LEP = get_unique_dist_g(LEP, iter=iter)

    IterationStep = 0
    get_new_tensor_iter=5
    iters = []
    taus = []
    ...
    torch.save(sequence_tensor, 'data/' + str(iter) + '_(' + str(LEP.g(0)[0]) + ',' + str(LEP.g(0)[1]) + ')_truncnorm_bridge2D_tensor_100_sequence.pt')
...
if __name__=='__main__':
    ...
    for i in range(2):
        sequence_tensor, LEP = sample_opti_sequenz(maxIteration=500,iter=1)
